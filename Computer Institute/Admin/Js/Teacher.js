...
$(document).on('click', '.Edit', function(){
    ...
    fetch(`PHP/Team/GetData.php?UpdateID=${UpdateID}`)
    .then((Response) => Response.json())
    .then((Data) => {
        $('#UPImg').attr('src', 'Img/Team/' + Data[0].Team_Img);
        $('#StudentID').val(Data[0].Team_ID );
        $('#OldImg').val(Data[0].Team_Img);
        $('#UP_Name').val(Data[0].Team_Name);
        $('#UP_Course').val(Data[0].Team_Position);
    })
})
        ...
        if (Data.Status == 'Empty'){
            ...
            for (let i = 0; i < Data.length; i++) {
                StudentsHTML += `<div class="Member">
                                    <img src="Img/Team/${Data[i].Team_Img}" alt="Team_member">
                                    <p class="Member_Name">${Data[i].Team_Name}</p>
                                    <p class="Member_Position">${Data[i].Team_Position}</p>
                                    <div class="Buttons">
                                        <button DeleteID="${Data[i].Team_ID}" class="Delete">Delete</button>
                                        <button UpdateID="${Data[i].Team_ID}" class="Edit">Edit</button>
                                    </div>
                                </div>`;
                
            }
            AllTeam.innerHTML = StudentsHTML;
        }
    })
}
LaodData();
...
$(document).on('click', '.Delete', function(){
    ...
    fetch(`PHP/Team/Delete.php?DeleteID=${DeleteID}`)
    .then((Response) => Response.json())
    .then((Data) => {
        if (Data.Status == 'Success') {
            ...
            ShowMessage(Error, `Best Teacher, I can't delete this`);
        }else{
